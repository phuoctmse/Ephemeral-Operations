...
export class UnauthorizedInstanceTypeError extends Error {
  constructor(instanceType: string) {
    super(`Instance type "${instanceType}" is not allowed. Only t3.micro and t4g.nano are permitted.`);
...
export class ConcurrencyLimitError extends Error {
  constructor(max: number) {
    super(`Maximum concurrent environments (${max}) reached. Please destroy an existing environment first.`);
...
export class TtlExceededError extends Error {
  constructor(requestedTtl: number, maxTtl: number) {
    super(`Requested TTL of ${requestedTtl}h exceeds maximum allowed of ${maxTtl}h. It will be overridden.`);
